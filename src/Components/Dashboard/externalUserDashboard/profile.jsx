...
export function Profile({ token }) {
    ...
    useEffect(() => {
        const fetchUserDetails = async () => {
            try {
                const response = await fetch('http://127.0.0.1:8080/api/v1/user/profile/YOUR_USER_ID', {
    ...
    const handleUpdateDetails = async () => {
        try {
            const response = await fetch('http://127.0.0.1:5000/api/v1/user/profile/YOUR_USER_ID', {
                method: 'PUT',
                headers: {
